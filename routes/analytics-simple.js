...
router.get('/deployments', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({
      deployments: Object.values(dailyStats),
      summary: {
...
router.get('/performance', authMiddleware, async (req, res) => {
  try {
    ...
    const avgDeploymentTime = deployments.length > 0
      ? deployments.reduce((sum, d) => sum + (d.deploymentTime || 0), 0) / deployments.length
      : 0;
    
    res.json({
      performance: {
        avgDeploymentTime: Math.round(avgDeploymentTime),
